...
{
​
  //Linear search - returns index of target in ArrayList al, or -1 if not found
  public static int linearSearch( ArrayList al, int target )
  {
    for( int pos=0; pos<al.size(); pos++) {
      if (al.get(pos).equals(target))  //if item at index pos is equivalent to the target, return pos
        return pos;
    }
    return -1; //return -1 if for loop did not find the item
  }//end foo()
​
​
  //Returns an ArrayList with numItems items where each item is a random integer in the range lo (inclusive) to hi (exclusive).
  public static ArrayList getRandomIntegers(int numItems, int lo, int hi)
  {
...
​
    for(int i=0; i<numItems; i++) {
	  ...
	  //Java will convert from int to Integer
...
​
  ...
  public static ArrayList wooHa(int numItems, int lo, int hi)
  {
    ArrayList retArr = new ArrayList();
​
    //If numItems specified is less than 1, return an empty ArrayList
    if ( numItems<1 )
	{
		//Print for info
